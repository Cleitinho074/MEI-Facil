...
app.delete('/api/sales/:id', auth, async (req, res) => {
  ...
  try {
    await client.query('BEGIN');
    // Remove cashflow entries linked to this sale
    await client.query(
      'DELETE FROM cashflow_entries WHERE sale_id=$1 AND user_id=$2',
      [req.params.id, req.userId]
    );
    // Remove sale items
    await client.query(
      'DELETE FROM sale_items WHERE sale_id=$1 AND user_id=$2',
      [req.params.id, req.userId]
    );
